...
namespace Mediamart.APIFORAPP
{
    ...
    public class DefaultController : ApiController
    {
        ...
        {
            var check = db.SendCodes.Where(a => a.Code == OTP && a.Phone == Phone).FirstOrDefault();
            if (check != null)
            {
        ...
        public HttpResponseMessage Warranti(WarrantiReq warranti)
        {
            ...
            try
            {
                ...
                var _old = db.Warrantis.Where(a => a.Phone == warranti.Phone && a.Createdate >= today && a.Createdate <= todayas);
                ...
                {
                    result.Status = 1;
                    result.Message = "Chỉ được tạo 1 phiếu bảo hành trong ngày.";
                    result.Data = new List<ProductResult>();
                    return ResponseMessage(result);
                }
                //tao hoac update thong tin khach hang
        ...
        public HttpResponseMessage SearchWarranti(string phone)
        {
            ...
            if (warranti.Count() > 0)
            {
                var _warr = warranti.FirstOrDefault();
                ...
                result.Status = 1;
                result.Message = "Thông tin phiếu bảo hành mới nhất của số điện thoại này.";
            ...
            {
                result.Status = 1;
                result.Message = "Không tìm thấy thông tin bảo hành của số điện thoại này.";
