...
using Mediamart.APIFORAPP.Model_Teka;
using Mediamart.APIFORAPP.Model_Teka.Model_Request;
using Mediamart.APIFORAPP.ModelRequest;
...
namespace Mediamart.APIFORAPP
{
    ...
    public class AppController : ApiController
    {
        ...
        public HttpResponseMessage GetActive(string serial)
        {
            try
            {
                var product = db.Products.Where(a => a.Code == serial).FirstOrDefault();
        ...
        [Route("getreportwarranti")]
        [HttpGet]
        public HttpResponseMessage GetReportWarranti(string UserId)
        {
            var cr_user = db.AspNetUsers.Find(UserId);
            var model = new List<RPWarranti>();
            var news = new RPWarranti()
            {
                Title = "Mới tạo",
                Status = 0,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 0).Count()
            };
            var success = new RPWarranti()
            {
                Title = "Hoàn thành",
                Status = 1,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 1).Count()
            };
            var station = new RPWarranti()
            {
                Title = "Chuyển trạm",
                Status = 2,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 2).Count()
            };
            //var reback = new RPWarranti()
            //{
            //    Title = "Thu hồi",
            //    Status = 3,
            //    Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 3).Count()
            //};
            var station_out = new RPWarranti()
            {
                Title = "Trạm từ chối",
                Status = 4,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 4).Count()
            };
            var pending = new RPWarranti()
            {
                Title = "Đang xử lý",
                Status = 0,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 5).Count()
            };
            var back_station = new RPWarranti()
            {
                Title = "Đem về trạm",
                Status = 6,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 6).Count()
            };
            var wait_accessary = new RPWarranti()
            {
                Title = "Đợi linh kiện",
                Status = 7,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 7).Count()
            };
            var wait_feedback = new RPWarranti()
            {
                Title = "Chờ phản hồi",
                Status = 8,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 8).Count()
            };
            var cancel = new RPWarranti()
            {
                Title = "Huỷ bỏ",
                Status = 9,
                Quantity = db.Warrantis.Where(a => a.Station_Warranti == cr_user.UserName && a.Status == 9).Count()
            };
            model.Add(news);
            model.Add(success);
            model.Add(station);
            model.Add(back_station);
            model.Add(station_out);
            model.Add(pending);
            model.Add(wait_accessary);
            model.Add(wait_feedback);
            model.Add(cancel);
            var result = new ReportWarrantiApi()
            {
                Status = 0,
                Message = "OK",
                Data = model
        ...
        public HttpResponseMessage GetDetail(long Id)
        {
            ...
            var lsDevice = from a in db.Warranti_Accessary
                           where a.WarrantiId == Id
                           select new PhuTung()
                           {
                               ID = a.Id.ToString(),
                               IDWarranti = a.WarrantiId,
                               IDPhutung = "",
                               Name = a.ProductName,
                               Quantity = a.Quantity,
                               Amount = a.Amount,
            ...
            var lisImage = new List<Warranti_Image>();
            foreach(var item in lsImage)
            {
                item.Image = ConfigControl.DOMAIN + item.Image;
                lisImage.Add(item);
            }
            var model = new UpdateDetailReq()
            {
                ...
                CheckIns = lisImage.Where(a => a.Type == 1).Select(a => a.Image).ToList(),
                Long = warranti.Long,
                Lat = warranti.Lat,
                Note = warranti.Note,
                Devices = lsDevice.ToList(),
                CheckOuts = lisImage.Where(a => a.Type == 2).Select(a => a.Image).ToList(),
        ...
        public HttpResponseMessage Warranti(TK_Warranti warranti)
        {
            try
            {
                ...
                var todayas = today.AddDays(1);

                var _old = db.Warrantis.Where(a => a.Phone == warranti.Phone && a.Createdate >= today && a.Createdate <= todayas && a.ProductCode == warranti.Serial);
                ...
                if (_old.Count() > 0)
                {
                    ...
                    {
                        Status = -1,
                        Message = "Chỉ được tạo 1 phiếu bảo hành trong ngày",
                        Data = new List<string>()
                    };
                    return ResponseMessage(resulter);
                }
                ...
                //tao thong bao
                var staff = db.AspNetUsers.Where(a => a.AspNetRoles.FirstOrDefault().Id == "Staff");
                foreach(var item in staff)
                {
                    ...
                    Notification notif = new Notification()
                    {
                        ...
                        Createdate = DateTime.Now,
                        Title = String.Format(TitleNotif.sent_station, _warranti.Code),
                        DetailsURL = String.Format(TitleNotif.url_warranti, _warranti.Id)
                    };
                    signal.Save(notif);
                }                
                ...
                {
                    Status = 1,
                    Message = "Yêu cầu bảo hành đã được ghi nhận, chúng tôi sẽ liên hệ lại trong vòng 24h. Liên hệ 19006788 khi cần hỗ trợ thêm. Cảm ơn quý khách",
                    Data = new List<string>()
                };
                return ResponseMessage(result);
            }
        ...
        {
            string active_success = "Sản phẩm {0} đã kích hoạt thành công từ {1} đến ngày {2}. Liên hệ 19006788 khi cần hỗ trợ thêm. Cảm ơn quý khách";
            ...
            try
            {
                ...
                if (product == null)
                {
                    ...
                    {
                        Status = -1,
                        Message = "Mã cào sản phẩm không tồn tại",
                        Data = new List<string>()
                    };
                    return ResponseMessage(resulter);
                }
                else if (product.Status == 1)
                {
                    ...
                    {
                        Status = -1,
                        Message = "Mã cào sản phẩm đã được kích hoạt trước đó",
                        Data = new List<string>()
                    };
                    return ResponseMessage(resulter);
                }
        ...
        public HttpResponseMessage Update(UpdateDetailReq detailReq)
        {
            try
            {
                ...
                logger.Info(json);
                string msg = "Cập nhật trạng thái ";
                ...
                var _phutung = db.Warranti_Accessary.Where(a => a.WarrantiId == warranti.Id);
                if (_phutung.Count() == 0)
                {
                    if (detailReq.Devices.Count() > 0)
                    {
                        ...
                        {
                            var a_key = db.A_Station.Where(a => a.StationId == warranti.Station_Warranti).SingleOrDefault(a => a.Name == item.Name);
                            if (a_key != null && (a_key.CountImport - a_key.CountExport) > 0)
                            {
                                ...
                                Notification notif = new Notification()
                                {
                                    SentTo = warranti.Station_Warranti,
                                    Createdate = DateTime.Now,
                                    Title = String.Format(TitleNotif.ss_warranti, warranti.Code),
                                };
                                signal.Save(notif);
                            }
                            else
                            {
                                msg = msg + "Linh kiện " + a_key.Name + " không còn ở trạm. Vui lòng liên hệ 19006788.";
                                warranti.Status = 7;
