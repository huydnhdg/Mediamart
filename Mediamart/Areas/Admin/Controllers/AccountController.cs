    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    //đoạn này thêm vào để cover cho User current lấy thêm Id FirstName LastName
                    var user = db.AspNetUsers.FirstOrDefault(a => a.UserName == model.UserName);
                    CustomPrincipalSerializeModel serializeModel = new CustomPrincipalSerializeModel();
                    serializeModel.Id = user.Id;
                    serializeModel.FirstName = user.FullName;
        ...
        public ActionResult Register()
        {
            if (User.IsInRole("Admin"))
            {
                ...
                TempData["parent"] = db.AspNetUsers.Where(a => a.AspNetRoles.FirstOrDefault().Id == "Key").ToList();
            }
            else
            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var hub = db.Station_Level.FirstOrDefault(a => a.S1 == model.Zone);
                    if (hub != null)
                    {
                        _user.Zone = model.Zone;
                        _user.Hub = hub.S2;
                        _user.HO = hub.S3;
                    }
                    else
