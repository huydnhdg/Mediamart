using Mediamart.Areas.Admin.Controllers;
    ...
    public class TypeErrController : BaseController
    {
        ...
        public ActionResult Index(int? page, string textsearch, string chanel, string status, string start_date, string end_date, string im_start_date, string im_end_date)
        {
            ...
            if (!string.IsNullOrEmpty(textsearch))
            {
                ...
                || a.Reason.Contains(textsearch)
                || a.Overcome.Contains(textsearch)
                || a.Description.Contains(textsearch));
                ViewBag.textsearch = textsearch;
            }
            if (!string.IsNullOrEmpty(chanel))
            {
                model = model.Where(a => a.Description.Contains(chanel));
        ...
        [Permission(Module = "typeerr.edit")]
        [HttpPost]
        ...
        [Permission(Module = "typeerr.delete")]
        ...
        [HttpPost]
        public JsonResult GetNote(string text)
        {
            var cate = (from a in db.Type_Err
                        where a.Description.Contains(text)
                        select new { a.Description });
            return Json(cate, JsonRequestBehavior.AllowGet);
        }
        [Permission(Module = "typeerr.upload")]
                ...
                if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
                {
                    ...
                    using (var package = new ExcelPackage(file.InputStream))
                    {
                        ...
                        for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                        {
                            ...
                            try { reason = workSheet.Cells[rowIterator, 4].Value.ToString(); } catch (Exception) { reason = ""; }
                            try { overcome = workSheet.Cells[rowIterator, 5].Value.ToString(); } catch (Exception) { overcome = ""; }
